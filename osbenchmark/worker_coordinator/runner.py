...
class BulkIndex(Runner):
    ...
    async def __call__(self, opensearch, params):
        ...
        * ``request-timeout``: a non-negative float indicating the client-side timeout for the operation.  If not present, defaults to
        ...
        if "BenchmarkAsyncOpenSearch" in str(opensearch):
    ...
    def simple_stats(self, bulk_size, unit, response):
        ...
        response_bytes_io = response if isinstance(response, BytesIO) else BytesIO(json.dumps(response).encode())
            ...
            try:
                for page in range(total_pages):
                    if page == 0:
                        ...
                        response_bytes_io = r if isinstance(r, BytesIO) else BytesIO(json.dumps(r).encode())
